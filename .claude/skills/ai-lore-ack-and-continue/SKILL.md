...
description: "Light mid-execution commit — same commit shape as `ack`, minimal ceremony, session continues"
---
...
A commit on the mounted track's branches — `trunk` on home, or `track/<name>` on a child track — in each dirty repo, with a brief Human-Lead-confirmed message. Both repos commit together as one unit; if only one repo is dirty, only that repo commits.
