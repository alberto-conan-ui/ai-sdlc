...
4. **Read the dials and posture** — `status/status.index.md` carries both. The session operates within them until a posture verb (`plan`/`reshape`/`execute`) or [`redial`](./redial.md) changes them. See [`status.md`](../../../.ai-lore-ai-sdlc/process/status.md).
