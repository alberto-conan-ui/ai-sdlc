...
`redial` re-attunes the session — to its conversational register (the two dials) and to the shape of the work it is currently doing (the active focus). Definitions for the dials and the postures live in [`status.md`](../status.md); this document is the operation that applies the re-attunement.
...
1. **Re-read [`status.md`](../status.md) and the active focus.** Definitions plus the work at hand. If no new setting is named, re-apply what status currently records.
