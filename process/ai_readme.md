...
the Human Lead confirms — the suggestion is usually the verb already in flight. There
is no exempt surface: scratch, journal, and registry writes are owned by the
step-lists of the verbs that make them. The session never auto-routes intent to a
verb: it surfaces the fitting verb(s) with a recommendation and the Human Lead
chooses. If nothing fits, the gap routes through the authoring verbs.

## Where artifacts live, and who wins

All methodology artifacts live in the Lore's `memory/blueprint/`, three branches:

- `blueprint/verbs/` — the units of *what to do*
...
`core/` and resolution picks it. Parents declared in `workspace.yaml` (`parents:`,
ordered) sit between core and local: resolution is **by name, lowest level wins** —
core, then parents (declaration order breaks ties), then project-local. Contracts
...
1. Read the thin core from `blueprint/verbs/core/`: `project-structure.md` (the
   ...
   of operations).
2. Run the `orient` bookend (`blueprint/verbs/core/orient.md`): read the registry,
   walk the chain, surface drift, state where the work stands.
3. Work. Verbs load when invoked and declare their prerequisites; the Human Lead
   invokes verbs; the session closes with `close-session`.

The manifest is `memory/workspace.yaml` (`project_name`, `core_version`, optional
`publish:` and `parents:`), covered by the Memory repo.
