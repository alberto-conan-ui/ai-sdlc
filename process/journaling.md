...
The methodology maintains a persistent record of what happened, what was learned, and what was decided. This document defines the recording system: what gets written, where it goes, and who writes it.
...
|---|---|
| **Date** | YYYY-MM-DD |
| **Stance** | which stance was active (if applicable) |
| **Mode** | Planning / Executing / Reflecting (if focused) |
...
**Handover section** — always present, always last. A targeted message for the next session working on this focus. See [The Handover](#the-handover) below.
...
| **Stance** | Architect |
| **Mode** | Executing |
| **Active focus** | Auth middleware rewrite |
...
**Audience:** The next session working on this focus. The handover is focus-scoped, not project-wide. It speaks to whoever picks up this thread of work next.
...
**Relationship to status.md:** Status.md is updated every session (mutable). The handover is written once (append-forward). They're complementary — status.md gives the project-wide picture, the handover gives the focus-specific thread.
...
**Focus context:** Journal entries naturally reference which focus they're working on. The header metadata links to the active focus. When you need to reconstruct the history of a specific focus, search the journal by focus name or follow the "Relevant journal" links from the focus file or AT index.

### The notepad (`knowledge-tree/notepad/`)

Action-scoped observations — things you notice during execution that don't belong in the temporal journal or in curated KT nodes yet. A pattern worth revisiting, a "this will matter later" note. The notepad captures *what you noticed* while doing other work, without forcing you to decide where the observation ultimately belongs.
...
On action completion, durable findings in the notepad migrate to domain KT nodes. The notepad node archives with the action. See [knowledge-tree.md — Notepad Branch](./knowledge-tree.md#notepad-branch) for the full lifecycle.

### The knowledge tree

Direct contributions — insights placed at the right node. The AI writes to the KT when an insight is immediately clear and well-placed. No intermediary needed.

**The boundaries.** All session narrative goes to the journal. Action-scoped observations go to the notepad. Curated, durable insights go to the knowledge tree. The action tree holds intention and gates — never narrative, observations, or design knowledge. All knowledge lives in the KT from day one, referenced from the AT.
...
1. **AI proposes status.** The active stance proposes the full status update: mode, active focus, next step, relevant journal links. The Human Lead confirms or corrects. This leverages the AI's full session context rather than asking the human to reconstruct it.

2. **Write the journal file** in `journal/live/YYYY-MM-DD_NN.md`:
   - Header metadata table (date, stance, mode, active focus)
...
3. **Update `status.md`:** Current state summary, relevant journal references, next step, mode, active focus. Use the status the Human Lead confirmed.
...
- [ ] `status.md` and `action-tree.index.md` updated — focus completed, status achieved?
...
- Periodically, files that have been reviewed move to `journal/archive/`.
- The Human Lead triggers the roll — either manually or by asking the AI to process the journal. Processing means: review `live/`, extract insights to the trees, then move processed files to `archive/`.
- Files that span the boundary are fine to archive — the action tree and knowledge tree hold the durable information. The archive preserves the historical record if you need to go back.
...
All journal files follow the append-forward principle unconditionally (see [updating-trees.md](./updating-trees.md)). Journal files are never edited or deleted after they're written — not even during reconciliation. The journal is the audit trail; it records transformations, it is never itself transformed. Old journal files move to `archive/` through normal rolling, never through editing or deletion.
