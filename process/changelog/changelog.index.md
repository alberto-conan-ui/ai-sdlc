...
| [v0.6.1](./v0.6.1.md) | Draft             | —              | Process polish: `ack` decoupled from `close-session` (new `ack-and-continue` light sibling for mid-execution chunking, close-session commits its own writes); focuses gain optional `claim` field, master's claim becomes focus-derived; Gemini binding section added to `bindings.md` parallel to Claude. |

**Current version:** v0.6 (latest release). v0.6.1 in draft.
